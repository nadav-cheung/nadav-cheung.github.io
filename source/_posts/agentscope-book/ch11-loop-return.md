...
chapter: 11
description: "ReAct 循环并非直接把工具结果返回给用户，而是让 Agent 再次推理并用自然语言作答。AgentScope 的 `reply()` 方法将推理、行动和总结串联为最多 10 轮的迭代循环，内置记忆压缩以防止上下文过长，并支持结构化输出。循环在模型返回纯文本、成功生成结构化输出或达到最大迭代次数时退出，后者会触发紧急总结。"
categories:
  - 算法与数据结构
tags:
  - ReAct循环
