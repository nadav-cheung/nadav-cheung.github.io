...
sticky: 1000
abbrlink: 216d1c97
description: "本题要求将两个逆序存储非负整数的链表相加，返回同格式的和链表。核心思路是同时遍历两链表，逐位相加并处理进位：用虚拟头节点统一操作，每次计算当前位和与进位，生成新节点链接。最后若仍有进位需追加节点。时间复杂度O(max(n,m))，空间复杂度相同，关键在于链表遍历与进位逻辑的协同处理。"
---
