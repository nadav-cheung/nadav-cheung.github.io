...
sticky: 1001
abbrlink: af104c4
description: "本文解析了 LeetCode 第 4 题“寻找两个正序数组的中位数”的 O(log(m+n)) 解法。核心思路并非合并数组，而是在较短的数组上进行二分查找，寻找满足 `nums1[i-1]≤nums2[j]` 且 `nums2[j-1]≤nums1[i]` 的切分点 i 与对应的 j，从而将两数组划分为左右两组。根据总长度的奇偶性，中位数取左侧最大值与右侧最小值的平均值或仅取左侧最大值，巧妙地将时间复杂度降至 O(log(min(m, n)))。"
---
